#http://bbs.fishc.com/thread-78199-1-1.html
...
request = urllib.request.urlopen(
    'http://api.map.baidu.com/direction/v1?mode=%s&origin=%s&destination=%s&origin_region=%s&destination_region=%s&output=json&tactics=%s&ak=%s'
...
if cxfs_id == '1':
    while flag == True:
        try:
            ...
            try:
                ...
                while flag == True:
                    try:
                        print(datas['result']['origin']['cityName'],
                              datas['result']['origin']['content'][step_count]['name'], '地址:',
                              datas['result']['origin']['content'][step_count]['address'])
                        step_count += 1
                    except:
                        flag = False
                        if step_count == 0:
                ...
                while flag == True:
                    try:
                        print(datas['result']['destination']['cityName'],
                              datas['result']['destination']['content'][step_count]['name'], '地址:',
                              datas['result']['destination']['content'][step_count]['address'])
                        step_count += 1
                    except:
                        flag = False
                        if step_count == 0:
    ...
    while flag == True:
        try:
            ...
            try:
                print('(首班车：%s，末班车：%s)' %
                      (datas['result']['routes'][0]['scheme'][0]['steps'][step_count][0]['vehicle']['start_time'],
                       datas['result']['routes'][0]['scheme'][0]['steps'][step_count][0]['vehicle']['end_time']))
            except TypeError:
                print('')
            else:
                print('')
            ...
            try:
                ...
                while flag == True:
                    try:
                        print(datas['result']['origin'][step_count]['name'], '地址:',
                              datas['result']['origin'][step_count]['address'])
                        step_count += 1
                    except KeyError:
                        flag = False
                        if step_count == 0:
                ...
                while flag == True:
                    try:
                        print(datas['result']['destination'][step_count]['name'], '地址:',
                              datas['result']['destination'][step_count]['address'])
                        step_count += 1
                    except:
                        flag = False
                        if step_count == 0:
    ...
    if datas['result']['routes'][0]['scheme'][0]['distance'] <= 1000:
        ...
        info = '以上方案距离为%d米，预计耗时%d秒，公交（地铁）票总价为%g元（%d分）' % (
        jvli, miao, datas['result']['routes'][0]['scheme'][0]['price'] / 100,
        datas['result']['routes'][0]['scheme'][0]['price'])
    elif datas['result']['routes'][0]['scheme'][0]['distance'] >= 1000:
        jvli = datas['result']['routes'][0]['scheme'][0]['distance'] / 1000
        miao = datas['result']['routes'][0]['scheme'][0]['duration']
        info = '以上方案距离为%g公里，预计耗时%d秒，公交（地铁）票总价为%g元（%d分）' % (
        jvli, miao, datas['result']['routes'][0]['scheme'][0]['price'] / 100,
        datas['result']['routes'][0]['scheme'][0]['price'])
